		...
		for fn in fns:
			with open(os.path.join(DATA_DIR, 
				...
				for row in tqdm.tqdm(csvr, desc="Loading flow info in {}...".format(fn)):
					...
					# 		for uid, v in self.domain_sni_uids_by_building_flows[building].items() if v <= n_delete]))
					# 	print("Deleting {} IDs".format(len(to_del_uids)))
					...
					# 		for uid, v in self.domain_sni_uids_by_hour_bytes[hour].items() if v <= n_delete]))
