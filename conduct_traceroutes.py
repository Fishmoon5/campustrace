...
class Traceroute_Conductor:
	...
	def _traceroute(self, outfn):
		...
		targets_sets = split_seq(self.targets, len(self.targets) // 1e3)
		for i,targets_set in enumerate(split_seq(target_sets, n_splits)):
	...
	def run(self):
		...
		warts_out_fn = glob.glob(os.path.join(out_dir, 'traces.warts'))
